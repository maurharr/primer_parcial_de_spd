...
void loop()
{
  ...
  if (valorLectura)//Si el interruptor retorna el valor "1"...
  {
    ...
    el funcionamiento de la función mostrarContador(multiplo)*/
    ...
    if(timer==20)//si el timer llega al valor "20", entra el if...
    {
      ...
      if(detectarMultiplos(contador))//Si el valor del contador es un multiplo de 2...
      {
        multiplo = contador;//la variable multiplo guarda el número del contador
      }
    ...
    mostrarContador(multiplo);//Muestra en los visualizadores, el número guardado por la variable multiplo 
  }
}
...
//------------------------------------
//declaro una función para determinar números multiplos de 2
bool detectarMultiplos(int numero)
{
  return (numero % 2 == 0);
}
...
void prendeDigito(int display)
{
	if (display == UNIDAD)//Si el valor del argumento es "UNIDAD"
    {
    	digitalWrite(UNIDAD, LOW);//prendo el visualizador de la unidad
      digitalWrite(DECENA, HIGH);//apago el visualizador de la decena
      delay(ANTIREBOTE);
    }
  	else if(display == DECENA)//Si el valor del argumento es "DECENA"
    {
